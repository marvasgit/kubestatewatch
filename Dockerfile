...
LABEL REPO="www.github.com/marvasgit/kubernetes-diffwatcher"

RUN apt-get update && \
    dpkg --add-architecture arm64 &&\
    apt-get install -y --no-install-recommends build-essential && \
    apt-get clean && \
    mkdir -p "$GOPATH/src/github.com/marvasgit/kubernetes-diffwatcher"

ADD . "$GOPATH/src/github.com/marvasgit/kubernetes-diffwatcher"

RUN cd "$GOPATH/src/github.com/marvasgit/kubernetes-diffwatcher" && \
